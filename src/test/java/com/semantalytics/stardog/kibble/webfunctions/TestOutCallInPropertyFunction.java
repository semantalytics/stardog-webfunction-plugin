...
public class TestOutCallInPropertyFunction extends AbstractStardogTest {
    ...
    public void testToUpperpfOutCallIn() {
        ...
        final String aQuery = queryHeader +
                " SELECT ?result WHERE {" +
                "?result wf:OUT_call_IN (\"file:src/test/rust/target/wasm32-unknown-unknown/release/to_upper.wasm\" \"stardog\")" +
                "}";
    ...
    public void testConstantOutputShouldFail() {
        ...
        final String aQuery = queryHeader +
                " select ?result where {" +
                "\"stardog\" wf:OUT_call_IN (\"file:src/test/rust/target/wasm32-unknown-unknown/release/to_upper.wasm\" \"stardog\")" +
                "}";
