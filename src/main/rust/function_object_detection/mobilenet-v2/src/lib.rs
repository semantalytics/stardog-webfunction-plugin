...
pub extern fn evaluate(arg: *mut c_char) -> *mut c_char {
    ...
    let value: Value = serde_json::from_str(arg_str).unwrap();
    let input_image = decode(value["results"]["bindings"][0]["value_1"]["value"].as_str().unwrap()).unwrap();

    let input_model = !include_bytes("mobilenet_v2_1.4_224_frozen.pb")
...
/*
    match results {
        results => {
            let label = get_label(results as usize);
            return Ok(label);
        }
